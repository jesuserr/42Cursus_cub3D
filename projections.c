...
/*   Updated: 2023/10/26 09:38:19 by jesuserr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ray_casting(t_cub *cub)
{
	...
	line.color = WHITE;
	draw_line(line, cub);
	square.x = horz.ray_x - WALL_SIZE / 8;
	square.y = horz.ray_y - WALL_SIZE / 8;
	square.color = WHITE;
	draw_square(cub, square, WALL_SIZE / 4);
	...
	line.color = 0xFF00FF;
	draw_line(line, cub);
	if (!(vert.ray_x > 2000 || vert.ray_y > 2000))
	{
		square.x = vert.ray_x - WALL_SIZE / 8;
		square.y = vert.ray_y - WALL_SIZE / 8;
		square.color = 0xFF00FF;
		draw_square(cub, square, WALL_SIZE / 4);
	}
}
...
void	check_horizontal_lines(t_ray_cast *horz, t_cub *cub)
{
	...
	while (horz->ray_number < 1)
	{
		...
		else
		{
			...
			horz->depth_of_field = 10;
		}
		while (horz->depth_of_field < 10)
		{
